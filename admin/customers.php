      ...
      <div class="table-responsive" style="padding-top:10px">
        <table class="table table-striped table-sm">
		  ...
		  if(isset($_POST["search"])){
			  $cat=$_POST["cat"];
			  $search=$_POST["name"];
			  $qry="SELECT * FROM users WHERE $cat like '%$search%' AND type=1 ORDER BY user_id DESC";
		}else{
		  
		  $qry="SELECT * FROM users WHERE type=1 ORDER BY user_id DESC";
			}
          $res = $pdo->query($qry);
          $counter=1;
		  if($res->num_rows > 0){
		  ...
		  ?>
            <tr>
              ...
              <td><a class="btn btn-sm btn-success" href="customer_view.php?user_id=<?php echo $row["user_id"];?>">View</a>
			  <a class="btn btn-sm btn-danger" href="../processes/user_delete.php?customer_id=<?php echo $row["user_id"];?>" role="button" onClick="return confirm('Do you really want to delete?');">Delete</a></td>
            </tr>
			<?php
			$counter++;
		  }
