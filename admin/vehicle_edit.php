...
<div style="padding-left:20px">
 <h1 class="jumbotron-heading">Edit Vehicles</h1>
</div>	

<div class="text-cente" style="margin-left:100px;margin-right:500px">
<form style="padding-top:50px" class="form-signin" action="../processes/vehicle_edit.php" method="POST" accept-charset="UTF-8" enctype="multipart/form-data">
			...
			?>
				...
				$id = $_GET['vehicle_id'];
				 $qrye = "SELECT * FROM vehicles WHERE vehicle_id= '$id'";
				  $rese = $pdo->query($qrye);
	  ...
	  <br />
	  <label>Model</label>
      <input type="text"  class="form-control" name="model" value="<?php echo $rowe['model'];?>" required >
	  <br/>
	  <label>Speed in Km/h</label>
	  <input type="text"  class="form-control" name="speed" value="<?php echo $rowe['speed'];?>" required >
	  <br/>
	  <label>Rim Size in Inches</label>
	  <input type="text"  class="form-control" value="<?php echo $rowe['rim'];?>" name="rim" required >
	  <br/>
	  <label>Weight in Kg</label>
	  <input type="text"  class="form-control" value="<?php echo $rowe['weight'];?>" name="weight" required >
	  <br/>
      <input type="hidden"  class="form-control" value="<?php echo $id;?>" name="vehicle_id" required >
	  
      <button class="btn btn-lg btn-primary btn-block" type="submit" name = "vehicle_edit" >Submit</button>
	  <br/>
    </form>
</div>
