...
<div style="padding-left:20px">
 <h1 class="jumbotron-heading">Update Company Info</h1>
</div>	

<div class="text-cente" style="margin-left:100px;margin-right:500px">
<form style="padding-top:50px" class="form-signin" action="../processes/company_edit.php" method="POST" accept-charset="UTF-8" enctype="multipart/form-data">
			...
			?>
				...
				$id = $_SESSION['id'];
				 $qrye = "SELECT * FROM users WHERE user_id= '$id'";
				  $rese = $pdo->query($qrye);
				$rowe=$rese->fetch_assoc();					
				?>
	  <label>Company Name</label>
      <input type="text"  class="form-control" name="name"  value="<?php echo $rowe['name'];?>" required autofocus>
	  <br />
	  <label>Email</label>
      <input type="text"  class="form-control" name="email" value="<?php echo $rowe['email'];?>" required >
	  <br/>
	  <label>Phone Number</label>
      <input type="text"  class="form-control" name="phone" value="<?php echo $rowe['phone'];?>" required >
	  <br/>
	  <label>City</label>
	  <input type="text"  class="form-control" name="city" value="<?php echo $rowe['city'];?>" required >
	  <br/>
	  <label>Location</label>
	  <input type="text"  class="form-control" name="location" value="<?php echo $rowe['location'];?>" required >
	  <br/>
	  
	  
      <button class="btn btn-lg btn-primary btn-block" type="submit" name = "company_edit" >Submit</button>
	  <br/>
    </form>
</div>
